...
// FILE NAME:dwsmath.h
// VERSION: 1.3.0
// FUNCTION: Dylan Schwilk's math template library
// REMARKS:
//----------------------------------------------------------------------------*/
// Copyright © 2002 */
/*----------------------------------------------------------------------------*/
...
// VER    | DATE | CHANGES */
//--------+------+---------------------------------------------+------------------*/
// 1.2.0  9/3/02   modified code to meet ansi standards and compile on mingw,
//                  cygwin, linux
// 2.0.0  4/18/03  Added Doxygen comments and removed non-templated math 
//                 functions to dws_numerical library (dws_numerical.h and 
//                 dws_numerical.cpp).  The dws_math library now consists only
//                 of the template header file dws_math.h.  I also 
//                 un-capitalized the utility functions (abs, etc) to conform 
...
//    #include <iterator>
     ...
     */
        ...
        template <class T>
        inline T median(T a, T b, T c){return ( max(a,max(b,c))==a ? max(b,c) : (c>b ? max(a,b):max(a,c) ));}
